...
calib_dir = os.path.join(script_dir, "fisheyes")
...
if not os.path.isdir(calib_dir):
    raise FileNotFoundError(
        "Fisheye calibration image folder not found. "
        f"Place fisheye chessboard images in: {calib_dir}"
    )

file_list = os.listdir(calib_dir)
img_file_list = sorted(
    file for file in file_list
    if file.startswith("Fisheye1") and file.lower().endswith((".jpg", ".jpeg", ".png", ".bmp"))
)

if not img_file_list:
    raise FileNotFoundError(
        "No fisheye calibration images found. "
        "Image filenames should start with 'Fisheye1' in the fisheyes folder."
    )
...
camera_mat_path = os.path.join(script_dir, "fisheye_camera_mat.npy")
dist_coefs_path = os.path.join(script_dir, "fisheye_dist_coefs.npy")
